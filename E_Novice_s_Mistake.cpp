...
frs(i,1,1000){
    rfr(j,i,i-100){
        lli check=i*n-j;
        string k=to_string(check);
        char c=k[0];
        bool bol=true;
        frs(i,1,k.size()-1){
            if(k[i]!= k[i-1])bol=false;
        }
        
        if(bol ==false || (c-'0')!=n)continue;
        if(k.size()==(i-j))ans.push_back({i,j});

    }
}
cout<<ans.size()<<'\n';
fr(i,ans.size())cout<<ans[i].ff<<' '<<ans[i].ss<<'\n';
}
