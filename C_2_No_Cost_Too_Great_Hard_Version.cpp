...
//    for(auto &it:t){
//         mm[it]--;
//         if(mm[it]==0)mm.erase(it);
//    }
   t=m.get_factorization(a[i]+1);
   for(auto &it:t){
      if(mm.count(it)){
        ans=min(ans,b[i]);
      }
...
//     cout<<it.ff<<' '<<it.ss<<'\n';
// }
 lli ind=min_element(all(b))-b.begin();
t=m.get_factorization(a[ind]);
   for(auto &it:t){
        mm[it]--;
        if(mm[it]==0)mm.erase(it);
   }
...
for(auto &it:mm){
  //  cout<<it.ff<<' ';
    ans=min(ans,b[ind]*(it.ff-a[ind]%it.ff));
}   
srt(b);
cout<<min(ans,b[0]+b[1])<<'\n';
